...
class ScrollView(StencilView):
    ...
    do_scroll = AliasProperty(
        _get_do_scroll, _set_do_scroll, bind=("do_scroll_x", "do_scroll_y")
    )
    ...
    vbar = AliasProperty(
        _get_vbar, None, bind=("scroll_y", "_viewport", "viewport_size")
    )
    ...
    hbar = AliasProperty(
        _get_hbar, None, bind=("scroll_x", "_viewport", "viewport_size")
    )
    ...
    def on_scroll_stop(self, touch, check_children=True):
        ...
        if self.do_scroll_x and self.effect_x:
            if not touch.ud.get("in_bar_x", False) and self.scroll_type != ["bars"]:
                self.effect_x.stop(touch.x)
        if self.do_scroll_y and self.effect_y and self.scroll_type != ["bars"]:
